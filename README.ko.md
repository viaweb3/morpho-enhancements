...
[app.morpho.org](https://app.morpho.org) 의 세 가지 빈자리를 채우는 Chrome 확장입니다:
...
│   ├── favorites.ts          # localStorage 기반 즐겨찾기 스토어
│   ├── graphql.ts            # blue-api 클라이언트
...
- Host permissions 는 `https://app.morpho.org/*` 으로 한정. 사용자가 구성한 RPC, `blue-api.morpho.org`, Morpho 의 토큰 로고 CDN 외에는 어떤 서비스로도 데이터를 보내지 않습니다.
