...
- **列表页收藏** —— 在 `/markets` 和 `/vaults` 给任意行打星标,再用一键 chip 把列表筛到只剩你关心的那些。数据只存 `chrome.storage.local`(无服务器、无追踪),通过 `chrome.storage.onChanged` 多 tab 同步,popup 也共享同一份数据。
...
# dashboard 挂载、暗色模式可读性、provider 桥、收藏星标 + 筛选,
...
└── e2e/
    ├── extension.spec.ts     # Lend 标签、dashboard 挂载、暗色、provider 桥
    ├── favorites.spec.ts     # 星标 + 过滤 + 持久化(走 postMessage 测试桥)
